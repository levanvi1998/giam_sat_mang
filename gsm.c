...
{
	//call print Ethernet
	print_ethernet_header2(Buffer , Size);
	...
	//print IP address
	fprintf(session , "IP: %s --> " , inet_ntoa(source.sin_addr) );
	fprintf(session , "%s\n" , inet_ntoa(dest.sin_addr) );
...
void print_tcp_packet2(const u_char * Buffer, int Size)
{
	//get time
	time_t now;
	time(&now);
	...
	int header_size =  sizeof(struct ethhdr) + iphdrlen + tcph->doff*4;
	if(tcp !=1) fprintf(session,"\n\n");
	
	//SYN-ACK
	if(((unsigned int)tcph->ack==1) && (unsigned int)tcph->syn==1)
	{
		fprintf(session , "%d. [SYN-ACK] [%02d:%02d:%02d]", tcp, hours, minutes, seconds);
		print_ip_header2(Buffer,Size);
		fprintf(session , "Port: %u --> %u",ntohs(tcph->source),ntohs(tcph->dest));
		return;
	}
	...
	{
		fprintf(session , "%d. [ACK] [%02d:%02d:%02d]", tcp, hours, minutes, seconds);
		print_ip_header2(Buffer,Size);
		fprintf(session , "Port: %u --> %u",ntohs(tcph->source),ntohs(tcph->dest));
	}
	...
	{
		fprintf(session , "%d. [SYN] [%02d:%02d:%02d]", tcp, hours, minutes, seconds);
		print_ip_header2(Buffer,Size);
		fprintf(session , "Port: %u --> %u",ntohs(tcph->source),ntohs(tcph->dest));
	}
